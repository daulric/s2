...
function useResolvedTheme(themeProp?: "light" | "dark"): Theme {
  useSignals();
  const detectedTheme = useSignal<Theme>(getDocumentTheme() ?? getSystemTheme());
  ...
  return themeProp ?? detectedTheme.value;
}
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const mapInstance = useSignal<MapLibreGL.Map | null>(null);
  const isLoaded = useSignal(false);
  const isStyleLoaded = useSignal(false);
  ...
  useImperativeHandle(ref, () => mapInstance.value as MapLibreGL.Map, [mapInstance.value]);
  ...
  useEffect(() => {
    ...
    const styleDataHandler = () => {
      ...
      styleTimeoutRef.current = setTimeout(() => {
        ...
        if (projection) {
          try {
            if (map.isStyleLoaded()) map.setProjection(projection);
          } catch { /* style internals not ready */ }
        }
      }, 200);
    };
    const loadHandler = () => { isLoaded.value = true; };
  ...
  useEffect(() => {
    if (!mapInstance.value || !isControlled || !viewport) return;
  ...
  }, [mapInstance.value, isControlled, viewport]);

  useEffect(() => {
    if (!mapInstance.value || !resolvedTheme) return;
  ...
  }, [mapInstance.value, resolvedTheme, mapStyles, clearStyleTimeout, isStyleLoaded]);

  const contextValue = useMemo(
    () => ({
      map: mapInstance.value,
      isLoaded: isLoaded.value && isStyleLoaded.value,
    }),
    [mapInstance.value, isLoaded.value, isStyleLoaded.value],
  );
  ...
  return (
    <MapContext.Provider value={contextValue}>
      ...
      >
        {(!isLoaded.value || loading) && <DefaultLoader />}
        {mapInstance.value && children}
      </div>
    </MapContext.Provider>
  );
});
...
function MapControls({
  ...
  useSignals();
  const { map } = useMap();
  const waitingForLocation = useSignal(false);
