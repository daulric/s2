...
The complete Supabase (PostgreSQL) database schema, covering all tables, columns, constraints, indexes, relationships, and Row Level Security policies.

The full migration lives in `frontend/sql/schema.sql`.

## Schema

All tables are created under the custom schema set by `NEXT_PUBLIC_SCHEMA`.
...
|-------|--------|---------------------|
| `profiles` | Public | Own row only (`auth.uid() = id`) |
| `videos` | Public if `visibility = 'public'`, otherwise owner only | Owner only |
| `video_likes` | Public | Own likes only |
| `subscribers` | Public | Own subscriptions only (`auth.uid() = subscriber`) |
| `audios` | Public if `visibility = 'public'`, otherwise owner only | Owner only |
| `stocks`, `stock_articles`, `article_sentiments`, `stock_predictions` | Public | Service role only |
| `user_watchlists` | Own watchlist only | Own watchlist only |
| `subscriptions` | Public | Service role only |

## Running the Migration

Open the Supabase SQL Editor and paste the contents of `frontend/sql/schema.sql`. The migration is idempotent -- all statements use `IF NOT EXISTS` or exception guards.
