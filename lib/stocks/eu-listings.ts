 ...
 * `||` header separators (FTSE), and tickers that need an exchange suffix
 * appended (LSE tickers like III -> III.L).
 */
function parseWikiTable(
  ...
  const headerLine = headerSeg.startsWith("!") ? headerSeg : headerSeg.replace(/^[\s\S]*?\n\s*!/, "!")
